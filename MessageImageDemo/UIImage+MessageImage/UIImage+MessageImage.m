...
- (BOOL)hh_isDecoded {
    ...
    NSNumber *num = objc_getAssociatedObject(self, @selector(hh_isDecoded));
    return [num boolValue];
}

- (void)setHh_isDecoded:(BOOL)hh_isDecoded {
    objc_setAssociatedObject(self, @selector(hh_isDecoded), @(hh_isDecoded), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (instancetype)hh_imageByDecoded {
    if (self.hh_isDecoded) return self;
    CGImageRef imageRef = self.CGImage;
    if (!imageRef) return self;
    CGImageRef newImageRef = hh_CGImageCreateDecodedCopy(imageRef, YES);
    ...
    newImage.hh_isDecoded = YES;
    return newImage;
}

CGColorSpaceRef hh_CGColorSpaceGetDeviceRGB() {
...
CGImageRef hh_CGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) {
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, hh_CGColorSpaceGetDeviceRGB(), bitmapInfo);
